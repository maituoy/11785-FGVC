...
# Criterion
config.loss.name = 'ce'
config.loss.label_smoothing = 0.0 #'num of steps to warmup LR, will overload warmup_epochs if set > 0')
...
# update_frequency
config.if_update_freq = False
config.update_frequency = 1
config.min_lr = 1e-7
config.weight_decay_end = None #Final value of the weight decay. We use a cosine schedule for WD and using a larger decay by the end of training improves performance for ViTs.
