...
class Bottleneck(nn.Module):
    def __init__(self, in_channels, out_channels, downsample=None, drop_path=0, stride=1):
        ...
        self.conv1 = nn.Conv2d(in_channels, in_channels, kernel_size=1, stride=1, padding=0, bias=False,  groups= in_channels)
        self.bn1 = nn.BatchNorm2d(in_channels)
        self.conv2 = nn.Conv2d(in_channels,  self.expansion*out_channels, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(self.expansion*out_channels)
        self.conv3 = nn.Conv2d(self.expansion*out_channels, self.expansion*out_channels, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn3 = nn.BatchNorm2d(self.expansion*out_channels)
...
class ResNet(nn.Module):
    def __init__(self, block, num_layers=18, dims=[96, 192, 384, 768], num_classes=7000, in_channels=3, drop_path=0.0):
        super().__init__()
...
def resnet50_c1_new(pretrained=False, progress=True, **kwargs):
